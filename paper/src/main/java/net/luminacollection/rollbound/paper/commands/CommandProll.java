...
public class CommandProll implements CommandsInterface
{
	...
	private final String COMMAND_NAME = Messages.byPath("command.proll.meta.name").toString();
	private final String COMMAND_ARGUMENT_THRESHOLD = Messages.byPath("command.proll.meta.argument.threshold").toString();
	private final String COMMAND_ARGUMENT_ROLL_STRING = Messages.byPath("command.proll.meta.argument.roll-string").toString();
	private final String COMMAND_SHORT_DESCRIPTION = Messages.byPath("command.proll.meta.short-description").toString();
	
	private static CommandProll instance;
	public static CommandProll instance()
	{
		if (instance == null) instance = new CommandProll();
		return instance;
	}
	private CommandProll()
	{
		for (int i = 0; i < 4; i++)
		{
			commands.add(new CommandAPICommand(COMMAND_NAME)
							 .withShortDescription(COMMAND_SHORT_DESCRIPTION)
							 .withPermission("rollbound.command.proll"));
		}
	...
	private void command1(CommandAPICommand command)
	{
		command.executesPlayer((player, args) -> {
			...
			rollManager.roll(player, roll, true);
		});
	}
